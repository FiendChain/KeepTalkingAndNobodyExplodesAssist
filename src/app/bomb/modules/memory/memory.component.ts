...
const buttonArrangement: number[] = [1,3,4,2];
...
function getButton(position: number): number {
    if(position <= 0 || position > buttonArrangement.length) {
        throw "Out of index error for buttonArrangement";
    } else {
        return buttonArrangement[position-1];
    }
}
...
export class MemoryComponent implements BombModuleInterface {
    ...
    public calculateOutput(input: number, stage: number): number {
        ...
        // stage 1
        case 1:
            if(input == 1) return getButton(2);
            if(input == 2) return getButton(2);
            if(input == 3) return getButton(3);
            if(input == 4) return getButton(4);
            break;
        // stage 2
        case 2:
            if(input == 1) return 4;
            if(input == 2) return this.getStage(1);
            if(input == 3) return getButton(1);
